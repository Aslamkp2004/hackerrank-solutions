...
int birthdayCakeCandles(int candles_count, int* candles) {
    int max_height = 0; // Initialize the maximum height to zero
    int count = 0;     // Initialize the count of tallest candles to zero

    // Iterate through the array to find the maximum height
    for (int i = 0; i < candles_count; i++) {
        if (candles[i] > max_height) {
            max_height = candles[i];
            count = 1; // Reset the count to 1 if a new maximum height is found
        } else if (candles[i] == max_height) {
            count++; // Increment the count if another candle with the same maximum height is found
        }
    }
